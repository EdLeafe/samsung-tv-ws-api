...
class SamsungTVAsyncArt(SamsungTVWSAsyncConnection):
    ...
    async def upload(
        ...
        if not data:
            _LOGGING.error(
                "upload: send_image request timed out (no 'ready_to_use' response in %ss). "
                "listener_alive=%s, connection_alive=%s",
                timeout,
                self._recv_loop is not None and not self._recv_loop.done(),
                self.is_alive(),
            )
            raise exceptions.ConnectionFailure(
                "TV did not respond to send_image request"
            )
        conn_info = json.loads(data["conn_info"])
        header = json.dumps(
            {
