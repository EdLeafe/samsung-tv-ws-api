...
from samsungtvws import SamsungTVWS, __version__, exceptions
...
def main():
    ...
    tokens = {
        "192.168.100.32": "token_file1.txt",
    ...
    if on:
        ...
        if supported:
            ...
            info = tv.art(
                10
            ).available()  # note the art(10) here sets the timeout to 10 seconds, not the normal 5 second timeout
            ...
            try:
                ...
                if (
                    ...
                    thumbs = tv.art().get_thumbnail(
                        content_id, True
                    )  # old api, gets thumbs in same format as new api
                else:
                    thumbs = tv.art().get_thumbnail_list(
                        content_id
                ...
                logging.info(
                    "got thumbnail for {} binary data length: {}".format(
                        content_id, len(thumb)
                    )
                )
            except Exception as e:
                logging.error(
                    "FAILED to get thumbnail for {}: {}".format(content_id, e)
                )
            ...
            if filename:
                content_id = tv.art(
                    ...
                    filename
                )  # note the art(10) here sets the timeout to 10 seconds, not the normal 5 second timeout
