...
def parseargs():
    ...
    parser = argparse.ArgumentParser(
        description="Async Change Mats for art on Samsung TV Version: {}".format(
            __version__
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="landscape mat to apply to art (use org to leave unchanged) (default: %(default)s))",
    )
    parser.add_argument(
        "-pm",
        "--pmat",
        ...
        type=str,
        default=None,
        help="portrait mat to apply to art (default: %(default)s))",
    )
    parser.add_argument(
        "-A",
        "--all",
        action="store_true",
        default=False,
        help="Apply to all art - usually just My-Photos (default: %(default)s))",
    )
    parser.add_argument(
        "-D",
        "--debug",
    ...
    if matte != None:
        if matte not in ["none", "org"]:
            ...
            if (mat not in mat_types) and (color not in mat_colors):
                logging.error(
                    f"Invalid matte type or color: {matte}. Supported matte types are: {mat_types}, colors: {mat_colors}"
                )
                sys.exit(1)
            return mat, color
    return matte, None
    ...
    if matte == "org" or (matte is None and color is None):
        ...
        if matte and color:
            ...
            if org_matte_id == "none":
                logging.warning(
                    f"can't change color/type of {'landscape' if landscape else 'portrait'} mat: {org_matte_id} for {art['content_id']} to {matte or color}"
                )
            else:
...
async def main():
    ...
    token_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), args.token_file
    )
        ...
        for art in available_art:
            try:
                ...
                if (
                    ...
                    or art["portrait_matte_id"] != portrait_target_matte_type
                ):
                    logging.info(
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Unable to change mats to {target_matte_type}/{portrait_target_matte_type} for {art['content_id']} ({art['width']}x{art['height']})"
                        )
            except KeyError:
