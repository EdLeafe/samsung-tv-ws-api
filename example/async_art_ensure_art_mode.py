...
from samsungtvws import __version__, exceptions
            ...
            while not self.exit:
                try:
                    ...
                    if tv_on:
                        # is art mode on
                        art_mode = (
                            await self.tv_art.get_artmode()
                        )  # calls websocket command to determine status
...
def parseargs():
    ...
    parser = argparse.ArgumentParser(
        description="Example async ensure art mode Samsung Frame TV Version: {}".format(
            __version__
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=15,
        help="How often to check in seconds (default: %(default)s))",
    )
    parser.add_argument(
        "-t",
        "--token_file",
