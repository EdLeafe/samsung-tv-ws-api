...
class ExifData:
    ...
    def load_data(self):
        try:
            with open(self.filename, "r") as f:
                data = json.load(f)
                [
                    self.exif[file].update(data[file])
                    for file in self.exif.keys()
                    if data.get(file)
                ]
        except Exception:
            pass
        ...
        try:
            with open(self.filename, "w") as f:
                data = {
                    file: {k: v}
                    for file in self.exif.keys()
                    for k, v in self.exif[file].items()
                    if k == "GEOPY_Address"
                }
                self.log.debug("SAVE:\r\n{}".format(pformat(data)))
                if data:
    ...
    def update_exif_dict(self, file):
        ...
        if HAVE_PIL and file not in self.exif.keys():
            ...
            self.exif[file] = {
                TAGS.get(tag, str(tag)): self.conv_bytes(value)
                for tag, value in (img._getexif() or {}).items()
            }
    ...
    def get_lat_long(self, file):
        ...
        if isinstance(gpsinfo, dict) and gpsinfo:
            ...
            lat = self._convert_to_degrees(gpsinfo[2]) * (
                -1 if gpsinfo[1].upper() != "N" else 1
            )
            lng = self._convert_to_degrees(gpsinfo[4]) * (
                -1 if gpsinfo[3].upper() != "E" else 1
            )
    ...
    def _convert_to_degrees(self, value):
        ...
        if value:
            val = [
                (lambda x, y: float(x / y))(*x) if isinstance(x, tuple) else x
                for x in value
            ]
    ...
    async def update_addresses(self, file_list):
        ...
        if HAVE_GEOPY:
            ...
            async with Nominatim(
                user_agent="{}-SamsungtvwsGetLocGallery".format(self.ip or ""),
                timeout=20,
                    ...
                    if "GEOPY_Address" not in self.exif[file].keys():
                        ...
                        if lat and lng:
                            ...
                            try:
                                ...
                                if locname:
                                    self.exif[file]["GEOPY_Address"] = (
                                        self.format_address(file, locname)
                                    )
                                ...
                                self.log.warning(
                                    "geocode failed on {}: {}".format(file, e)
                                )
    ...
    def format_address(self, file, locname):
        ...
        try:
            ...
            addr = [
                locname.raw["address"].get("village"),
