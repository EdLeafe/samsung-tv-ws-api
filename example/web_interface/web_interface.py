...
from quart import Quart, current_app, make_response, render_template, websocket
...
def parseargs():
    ...
    parser = argparse.ArgumentParser(
        description="Async Art gallery for Samsung Frame TV Version: {}".format(
            __version__
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="port for web page interface (default: %(default)s))",
    )
    parser.add_argument(
        "-f",
        "--folder",
    ...
    parser.add_argument(
        ...
        help="default token file to use (default: %(default)s))",
    )
    parser.add_argument(
        "-u",
        "--update",
    ...
    parser.add_argument(
        ...
        type=int,
        default=120,
        help="how long to display manually selected art for (default: %(default)s))",
    )
    parser.add_argument(
        "-mo",
        "--modal",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Show in Kiosk mode (default: %(default)s))",
    )
    parser.add_argument(
        "-P",
        "--production",
        action="store_true",
        default=False,
        help="Run in Production server mode (default: %(default)s))",
    )
    parser.add_argument(
        "-S",
        "--sequential",
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="Use Exif data (default: %(default)s))",
    )
    parser.add_argument(
        "-D",
        "--debug",
...
class WebServer:
    ...
    async def serve_forever(self, production=False):
        ...
        if production:
            ...
            await asyncio.gather(
                self.run(), self.tv.start_monitoring(), return_exceptions=True
            )
    ...
    async def get_template_attribute(self, template, attibute):
        ...
        """
        return getattr(
            await current_app.jinja_env.get_template(
                template
    ...
    async def broadcast_tv_filename(self):
        ...
        while not self.exit:
            ...
            for websoc in self.connected:
                ...
                ):  # skip if image was previously requested, as modal is already displayed
                    self.log.info(
                        "WS({}): Not sending {} as image was previously selected".format(
                            websoc.id, data["name"]
                        )
                    )
    ...
    async def ws_process(self, data):
        ...
        if data["type"] == "modal":
            ...
            if text:
                modal_window = await self.get_template_attribute(
                    "macros.html", "render_modal"
                )
            ...
            await self.ws_send({"type": "update", "name": name})
    ...
    async def ws_send(self, data, websoc=None):
        ...
        if not self.debug:
            self.log.info(
                "WS({}): sending: type: {}, name: {}".format(
                    ws.id, data.get("type"), data.get("name", data)
                )
            )
    ...
    async def ws(self):
        ...
        try:
            ...
            await self.ws_process(
                {"type": "refresh"}
            )  # send 'refresh' to update display on first connection
    ...
    async def show_thumbnails(self):
        ...
        image_names = [
            img
            for img in os.listdir(self.app.static_folder)
            if not img.upper().endswith(".TXT")
        ]
        self.exif.get_files(image_names)
        self.log.info("displaying Buttons for: {}".format(image_names))
        return await render_template(
            "home.html", names=image_names, kiosk=str(self.kiosk).lower()
        )
