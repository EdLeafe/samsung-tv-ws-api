...
class PIL_methods:
    ...
    async def check_thumbnails(self, files_images, my_photos):
        ...
        if my_photos_thumbnails:
            self.log.info(
                "checking thumbnails against {} files, please wait...".format(
                    len(files_images)
                )
            )
    ...
    def compare_thumbnails(self, files_images, my_photos_thumbnails):
        ...
        for k, (filename, file_data) in enumerate(files_images.items()):
            for i, (my_content_id, my_data) in enumerate(
                ...
                self.log.debug(
                    "checking: {} against {}, thumbnail: {} bytes".format(
        ...
        for file in files:
            try:
                ...
                if not (
                    ...
                    self.log.warning(
                        "file: {} is of type {}, the extension is wrong! please fix this".format(
                            file, format
                        )
                    )
...
class monitor_and_display:
    ...
    def __init__(
        ...
        self.period = (
            min(max(5, period), self.update_time, display_for)
        ...
        if self.matte != "none":
            ...
            try:
                ...
                if matte[0] in matte_types and matte[1] in matte_colors:
                    ...
                    self.log.info(
                        "Valid mattes types: {} and colors: {}".format(
                            matte_types, matte_colors
                        )
                    )
                self.log.warning(
                    "Invalid matte selected: {}. A valid matte would be shadowbox_polar for eample, using none".format(
                        self.matte
                    )
                )
    ...
    def get_file_type(self, filename, image_data=None):
        ...
        try:
            ...
            if file_type in self.allowed_ext:
                file_type = self.pil.fix_file_type(
                    filename, file_type, image_data
                )
                return file_type
        except Exception as e:
            self.log.error("Error reading file: {}, {}".format(filename, e))
                ...
                if self.uploaded_files.get(filename, {}).get("content_id"):
                    self.log.info(
                        "uploaded : {} to tv as {}".format(
                            filename,
                            self.uploaded_files[filename]["content_id"],
                        )
                    )
                else:
                    self.log.warning(
    ...
    async def update_art_timer(self):
        ...
        if self.update_time > 0 and (
            ...
            if time.time() - self.start >= self.update_time:
                self.log.info(
                    "doing slideshow update, after {}".format(
                        self.get_time(self.update_time)
                    )
                )
                self.start = time.time()
                self.write_program_data()
    ...
    async def change_art(self, new_content_id=None):
        ...
        if content_id and content_id != self.current_content_id:
            ...
            self.log.info(
                "skipping art update, as new content_id: {} is the same as currently shown".format(
                    content_id
                )
            )
    ...
    async def tv_in_artmode(self):
        try:
            ...
            self.log.warning(
                "AssertionError error: {} returning: {}".format(
                    e, self.tv.art_mode
                )
            )
            ...
            if await self.tv_in_artmode():
                self.log.info(
                    "checking directory: {}{}".format(
                        self.folder,
                        " every {}".format(self.get_time(self.period))
                        if self.period
                        else "",
                    )
                )
...
async def main():
    ...
    log.info(
        "This is a library for the web_interface, please run that instead"
    )
